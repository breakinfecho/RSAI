...
def extract_reviews(data):
    ...
    for entry in data:
        if entry.startswith("<<SYS>>"):
            # 提取预测、解释和特征
            line = entry.strip()
            prediction_explanation = line.split('features:')
            if len(prediction_explanation) > 1:
                prediction_and_explanation = prediction_explanation[0].split(', explanation:')
                if len(prediction_and_explanation) > 1:
                    prediction = prediction_and_explanation[0].split(': ')[1].strip()
                    explanation = prediction_and_explanation[1].strip()

                    # 提取特征并清理
                    features_str = prediction_explanation[1].strip().strip('].</SYS>').strip('[]')
                    features = [feature.strip() for feature in features_str.split(',') if feature]  # 去除空条目
...
class RewardModel(BaseRewardModel):
    ...
    def reward_calculate(self, task, input_field_data, title_list):
        ...
        # item_ranking = get_item_ranking(self.args.backup_ip, [user], [sub_sequential], [item_list],
        #                                 port=self.teacher_port)
        ...
        #                     if _ in self.category2item[target_category]:
        #                         task_score.append(1.0)
        #                     elif out_category_count < (
        #                             input_field_data['item_count'] - input_field_data['category_count']):
        #                         task_score.append(0.5)
        #                     else:
        #                         task_score.append(0.0)
        #
        ...
        # list_reward = rank_corrected_score.sum() / self.best_ranking_score[
        #     item_count] * ranking_score_frac + list_task_reward * task_score_frac  # NR-17-21
    ...
    def get_reward(self, batch, output_title_list, only_reward=False) -> RewardOutput:
        ...
        :return: RewardOutput
        """
        task, input_text, input_field_data = batch['task'] * self.args.sample_num, batch[
            'input_text'] * self.args.sample_num, batch['input_field_data'] * self.args.sample_num
        reward_data = [self.reward_calculate(t, ifd, tl) for t, ifd, tl in
                       zip(task, input_field_data, output_title_list)]
        item_reward, list_reward = [rd[0] for rd in reward_data], [rd[1] for rd in reward_data]
        if only_reward:
            return RewardOutput(reward=list_reward)
        _output_text = [get_output_text(tl, '\n' + self.tokenizer.eos_token, idx=self.args.idx) for tl in
                        output_title_list]
        ...
        if self.args.fine_grain_reward:
            ...
            for idx in range(len(task)):
                if self.args.reward_scale:
                    ...
                    score_scaling_factor = self.running['item'][task[idx]].std + torch.finfo(
                        _item_reward[idx].dtype).eps
                    _item_reward[idx] /= score_scaling_factor
                total_reward[idx][complete_data['input_ids'][idx] == 13] += _item_reward[idx]

        return RewardOutput(complete_data=complete_data, action_mask=action_mask, total_reward=total_reward,
                            reward=list_reward)
