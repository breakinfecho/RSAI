...
def getOut(can, rate):
    try:
        completion = client.chat.completions.create(
            model="/data/share/models/glm-4-9b-chat",
            messages=[
                {"role": "user", "content": "你是一个专业的语言分析专家。"},
                {"role": "user",
                 "content": "你需要只输出一个数字用来表示预测餐厅的生存状态（0-倒闭，1-存活）"
                            "（如果评论过于片面或激进时，请参考上述餐厅平均评分进行判断），"
...
client = OpenAI(
    base_url="http://127.0.0.1:50072/v1",
    api_key="123456",
)
print("服务连接成功")
count = 0
...
try:
    with open('/home/njf/Rec/data/City1/dataset_evl.pickle', 'rb') as f_check:
