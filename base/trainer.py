...
from transformers import get_polynomial_decay_schedule_with_warmup, LogitsProcessorList, StoppingCriteriaList, \
...
class BaseTrainer(nn.Module):
    def __init__(self, args):
        ...
        self.accelerator = Accelerator(
            ...
            kwargs_handlers=[DistributedDataParallelKwargs(find_unused_parameters=(self.args.train_stage == 'RL'))]
            # need for RL
        )
    ...
    def get_optimizer_scheduler(self, named_params, batch_per_epoch=None, group_wd_params=True):
        ...
        if self.args.train_stage == 'RL':
            scheduler = get_polynomial_decay_schedule_with_warmup(optim, 50 * self.accelerator.num_processes,
                                                                  batch_per_epoch * self.args.num_episodes, power=2.0)
        else:
        ...
        with self.accelerator.accumulate(self.warped_actor_critic):
            ...
            if self.accelerator.sync_gradients:
                ...
                if self.args.clip_grad_norm > 0:
                    total_norm = self.accelerator.clip_grad_norm_(self.optim.param_groups[0]['params'],
    ...
    def RL_sample_batch(self, batch):
        ...
        if self.args.sample_num > 1 and self.args.lr > 0:
            ...
            output_ids = self.actor_critic.actor_model.sample(
                **sample_input_data, logits_processor=self.logits_processor, logits_warper=self.logits_warper,
    ...
    def PPO_process_batch(self, complete_data, action_mask, total_reward):
        ...
        for i in range(0, total_reward.shape[0], 16):
            ...
            old_action_values = self.actor_critic.forward(scope=self.actor_critic.critic_lora_scope,
                                                          **temp_learn_data['complete_data'])

            old_sequence_logit = self.actor_critic.forward(scope=self.actor_critic.actor_lora_scope,
                                                           **temp_learn_data['complete_data']).logits
            ...
            old_sequence_log_probs_shifted = log_prob(old_sequence_dists_shifted,
                                                      temp_learn_data['complete_data']['input_ids'])
            ...
            ref_sequence_log_probs_shifted = log_prob(ref_sequence_dists_shifted,
                                                      temp_learn_data['complete_data']['input_ids'])
            ...
            for idx in range(mini_sample_num):
                ...
                (
                    ...
                    rearrange(complete_data['input_ids'][idx, ...][ignore_index][-max_len:],
                              '... -> 1 ...').contiguous(),
                    ...
                    rearrange(old_sequence_log_probs_shifted[idx, ...][ignore_index][-max_len:],
                              '... -> 1 ...').contiguous(),
                    rearrange(ref_sequence_log_probs_shifted[idx, ...][ignore_index][-max_len:],
                              '... -> 1 ...').contiguous(),
        ...
        for epoch in range(self.args.epoch):
            ...
            for (
                ...
                with self.accelerator.accumulate(self.warped_actor_critic):
                    ...
                    # self.accelerator.wait_for_everyone()
                    sequence_logit = self.warped_actor_critic.forward(self.actor_critic.actor_lora_scope, _sequence,
                                                                      attention_mask=_sequence != 0).logits
                    ...
                    # critic loss
                    action_values = self.warped_actor_critic.forward(self.actor_critic.critic_lora_scope, _sequence,
                                                                     attention_mask=_sequence != 0)
                    ...
                    if self.accelerator.sync_gradients:
                        ...
                        # self.accelerator.wait_for_everyone()
                        if self.args.clip_grad_norm > 0:
                            self.accelerator.clip_grad_norm_(self.optim.param_groups[0]['params'],
                                                             self.args.clip_grad_norm)
                    self.optim.step()
                    self.optim.zero_grad()
    ...
    def dataset_prepare(self):
        ...
        self.val_loader = DataLoader(self.val_data, batch_size=self.args.val_batch_size, shuffle=False,
                                     collate_fn=self.val_data.collate_fn, drop_last=False)
        ...
        for eps in range(self.args.num_episodes):
            ...
            for batch in self.train_loader:
                ...
                pbar.set_description(
                    f'RL learning in episode: {eps} | sample_batch: {self.sample_batch} | example: {len(self.memories)} | '
